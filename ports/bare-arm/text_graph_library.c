...
void clrendl(void){
  int i;
  for(i=0;i<twidth-((cursor-TVRAM)%twidth);i++){
    cursor[i] = 0;
  }
}
...
void printchar(unsigned char n){
  ...
  if(n=='\033'){
    ...
    if(escphase==1&&n=='['){
      escphase++;
      return;
    }else if(escphase==2&&n=='K'){
      clrendl();
      escphase = 0;//end of esc sequence.
      return;
    }else if(escphase == 2){
      if(n>='0'&&n<='9'){
	num *=10;
	...
	case 'B':
	  cursor += twidth*num;
	  if(cursor > TVRAM+twidth*(WIDTH_Y-1))
	    cursor = TVRAM+twidth*(WIDTH_Y-1);
	  break;
	case 'C':
	  cursor += num;
...
void printstr(const char *s){
	//カーソル位置に文字列sを表示
  while(*s){
    printchar(*(const unsigned char*)s++);
  }
}
